...
def diagram_feature(df):
    ...
    st.write('''
            ...
            - Положительную линейную зависимость, например, имеет пара CHILD_TOTAL - DEPENDANTS (что логично);
...
def diagram_with_target(df):
    ...
    st.write('''
            ...
            - Чем меньше PERSONAL_INCOME, тем больше информации об отклике/его отсутствии;
            - Для AGE высокие показатели по отклику/отсутствию отклика приходятся с 22 лет до 40 лет;
            Это показывает, кто может являться целевой таргет-группой кредитных программ банка.
                    ''')
...
def mattrix(df):
    ...
    st.write('''
        ...
        - Целевая переменная слабо коррелирует с выведенными признаками.
                ''')
...
if __name__ == "__main__":
    ...
    - LOAN_NUM_TOTAL — количество кредитов клиента;
    - LOAN_NUM_CLOSED — количество погашенных кредитов клиента.
                    ''')
